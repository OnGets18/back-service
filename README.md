...
Deploy to your EC2 instance using the automated script:

```bash
# Set your Docker Hub token (one time)
export DOCKER_HUB_TOKEN='your_docker_hub_token'

# Deploy to EC2 (default: ec2-user@98.84.25.68)
./deploy-to-ec2.sh

# Or specify custom EC2 details
./deploy-to-ec2.sh [EC2_IP] [EC2_USER] [SSH_KEY_PATH]

# Example with SSH key
./deploy-to-ec2.sh 98.84.25.68 ec2-user ~/.ssh/my-key.pem
```
...
**Prerequisites:**
- EC2 instance must have Docker installed
- Security group must allow SSH (port 22) and HTTP (port 3000)
- Docker Hub token must have read/write permissions

### Manual Deployment

For manual deployment steps, see [DEPLOY.md](./DEPLOY.md).
